...
public class GlobalExceptionHandler {
    ...
    /*
        @RestControllerAdvice로 등록된 전역 예외 처리 방식은 Controller에서 발생된 예외만 처리한다
        Service, Repository, Filter에서 발생하는 예외는 처리하지 못한다
        ExpiredJwtException처럼 security filter 단에서 발생하는 예외 타입은 애초에 요청 자체가
        Controller에 닿지 못하기 때문에 처리할 수 없다
